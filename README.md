# gltf [![PkgGoDev](https://pkg.go.dev/badge/github.com/amorist/gltf)](https://pkg.go.dev/github.com/amorist/gltf) [![Build Status](https://travis-ci.com/qmuntal/gltf.svg?branch=master)](https://travis-ci.com/qmuntal/gltf) [![Coverage Status](https://coveralls.io/repos/github/qmuntal/gltf/badge.svg?branch=master)](https://coveralls.io/github/qmuntal/gltf?branch=master) [![License](https://img.shields.io/badge/License-BSD%202--Clause-orange.svg)](https://opensource.org/licenses/BSD-2-Clause) [![Mentioned in Awesome Go](https://awesome.re/mentioned-badge-flat.svg)](https://github.com/avelino/awesome-go)
...
- [gltf.Document](https://pkg.go.dev/github.com/amorist/gltf#Document): Idiomatic glTF data model
- [gltf.Open](https://pkg.go.dev/github.com/amorist/gltf#Open) / [gltf.Save](https://pkg.go.dev/github.com/amorist/gltf#Save): `.glTF` and `.glb` [d]encoding support
- [qmuntal/gltf/ext](https://pkg.go.dev/github.com/amorist/gltf/ext): Built-in extensability mechanism with principal extensions provided out of the box
- [qmuntal/gltf/modeler](https://pkg.go.dev/github.com/amorist/gltf/modeler): Friendly package to read and write accessors and buffer views
- [qmuntal/gltf/binary](https://pkg.go.dev/github.com/amorist/gltf/binary): Friendly and efficient package to read and write bytes from buffers

## :scroll: Getting started

### Data Model

`qmuntal/gltf` implements the whole glTF 2.0 specification. The top level element is the [gltf.Document](https://pkg.go.dev/github.com/amorist/gltf#Document) and it contains all the information to hold a gltf document in memory:
...
A [gltf.Document](https://pkg.go.dev/github.com/amorist/gltf#Document) can be decoded from any `io.Reader` by using [gltf.Decoder](https://pkg.go.dev/github.com/amorist/gltf#Decoder):
...
When working with the file system it is more convenient to use [gltf.Open](https://pkg.go.dev/github.com/amorist/gltf#Open) as it automatically manages relative external buffers:
...
A [gltf.Document](https://pkg.go.dev/github.com/amorist/gltf#Document) can be encoded to any `io.Writer` by using [gltf.Encoder](https://pkg.go.dev/github.com/amorist/gltf#Encoder):
...
When working with the file system it is more convenient to use [gltf.Save](https://pkg.go.dev/github.com/amorist/gltf#Save) and [gltf.SaveBinary](https://pkg.go.dev/github.com/amorist/gltf#SaveBinary) as it automatically manages relative external buffers:
...
The package [gltf/modeler](https://pkg.go.dev/github.com/amorist/gltf/modeler) defines a friendly API to read and write accessors and buffer views, abstracting away all the byte manipulation work and the idiosyncrasy of the glTF spec.
...
The data of the attributes that are stored in a single bufferView may be stored as an Array-Of-Structures, which may produce a rendering perfomance boost in static attributes. `qmuntal/gltf/modeler` facilitates the creation of interleaved accessors and buffer views with the methods [WriteAttributesInterleaved](https://pkg.go.dev/github.com/amorist/gltf/modeler#WriteAttributesInterleaved), [WriteAccessorsInterleaved](https://pkg.go.dev/github.com/amorist/gltf/modeler#WriteAccessorsInterleaved), and [WriteBufferViewInterleaved](https://pkg.go.dev/github.com/amorist/gltf/modeler#WriteBufferViewInterleaved) being the first one the most recommended for creating mesh primitives:
...
The package [gltf/binary](https://pkg.go.dev/github.com/amorist/gltf/binary) defines a friendly and efficient API to read and write bytes from buffers, abstracting away all the byte manipulation work. This package is very low level and normal users should use `gltf/modeler` instead as it provides another level of abstraction that understands how bytes are associated to other entities.
...
- [draco-go/gltf/draco](https://github.com/amorist/draco-go)

#### Custom extensions

To implement a custom extension encoding, provide a `struct` that can be encoded as a JSON object as dictated by the spec.

To implement a custom extension decoding, call [gltf.RegisterExtension](https://pkg.go.dev/github.com/amorist/gltf#RegisterExtension) at least once before decoding, providing the identifier of the extension and a function that decodes the JSON bytes to the desired `struct`:
